...
public class SensorMessageSettingActivity extends BaseActivity implements CompoundButton.OnCheckedChangeListener {
    ...
    private TextView timeIntervalTextView, tipText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (serializable != null && serializable instanceof Device) {
            ...
            isSmartLock = ProductManage.isSmartLock(device);
    ...
    private void init() {
        if (isSmartLock) {
            navigationBar.setCenterText(getString(R.string.lock_back_home_tip));
            tipText.setText(getString(R.string.lock_back_home_tip));
        }
    ...
    private void refresh() {
        ...
        if (isSmartLock)
            messagePush = messagePushDao.selLockMessagePush(deviceId, doorUserData.getAuthorizedId());
        else
    ...
    public void onBackPressed() {
        ...
        if (!isSmartLock) {
            ...
            sensorTimerPush.startTimerPush(MessagePushType.DEVICE_SMART_LOCK, device.getDeviceId(), doorUserData.getAuthorizedId(), messagePush.getIsPush(), messagePush.getStartTime(), messagePush.getEndTime(), messagePush.getWeek());
        showDialogNow();
    }
