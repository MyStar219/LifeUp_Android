...
/**
 * @author Allen
 *         EditText With Left Or Right Compound Drawable.
 *         right compound drawable will be showToast as R.drawbale.edit_text_delete_icon if not define.
 */
public class EditTextWithCompound extends EditText implements TextWatcher, View.OnFocusChangeListener, MenuItem.OnMenuItemClickListener {
    ...
    private boolean needFilter = false;//是否需要过滤特殊字符,默认需要过滤
    private boolean needRestrict = false;//是否需要限制特殊,默认需要限制

    public static final int MAX_TEXT_LENGTH = 32;//设备名称、场景名称、房间名称命名长度放开至32个字母,16个汉字

    public static final int MAX_TEXT_LENGTH_COMMON = 16;//原有的一些名称限制为16位
    ...
    private void init() {
        ...
        rightfulBackgroundDrawable = getResources().getDrawable(R.drawable.edit_text_bg);
        unlawfulBackgroundDrawable = getResources().getDrawable(R.drawable.edit_text_error);
        if (rightDrawable == null) {
            rightDrawable = getResources().getDrawable(R.drawable.edit_text_delete_icon);
        }
...
//            setText(charSequence);
            ...
            if (needFilter) {
                str = EmojiFilterUtil.filterEmoji(editable.toString());
            }
            str = str.replaceAll("'", "");
            ...
            if (needRestrict && !StringUtil.isEmpty(str) && StringUtil.isHasSpecialChar(str)) {
...
//        LogUtil.d(TAG,"setRightful()");
    ...
    public void showDeleteDrawable() {
        rightDrawable = ViHomeApplication.getAppContext().getResources().getDrawable(
...
//    public void setUnlawfulBackgroundDrawable(Drawable unlawfulBackgroundDrawable) {
//        this.unlawfulBackgroundDrawable = unlawfulBackgroundDrawable;
//        setDrawable();
//    }
...
//    public void setPwdEyeEnable(boolean pwdEyeEnable) {
//        this.pwdEyeEnable = pwdEyeEnable;
//        if (pwdEyeEnable) {
//            rightDrawable = getResources().getDrawable(R.drawable.password_show);
...
//    }
//
//    public void disableDrawableRight() {
//        rightDrawable = null;
//        setDrawable();
//    }
            ...
            if (isFocused() && rightDrawable != null && event.getX() > (getWidth() - getPaddingRight() - rightDrawable.getIntrinsicWidth())
                ...
                if (pwdEyeEnable) {
                    if (getTransformationMethod() instanceof PasswordTransformationMethod) {
                        ...
                        rightDrawable = getResources().getDrawable(R.drawable.password_hide);
                    } else {
                        setTransformationMethod(PasswordTransformationMethod.getInstance());
                        rightDrawable = getResources().getDrawable(R.drawable.password_show);
                    }
