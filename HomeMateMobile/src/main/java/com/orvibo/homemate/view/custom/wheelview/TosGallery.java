 ...
 * <p>
 * Views given to the Gallery should use {@link com.orvibo.homemate.view.custom.wheelview.TosGallery.LayoutParams} as
...
public class TosGallery extends TosAbsSpinner implements
	...
	protected int computeHorizontalScrollExtent() {
		...
		if (count > 0) {
			...
			if (width > 0) {
				...
				// If the last child is showPopup, we do no add the right extent.
	...
	private void setSelectionToCenterChildVertical() {
		...
		Log.d(getClass().getName(), "newPos:"+newPos);
		if (newPos != mSelectedPosition) {
			setSelectedPositionInt(newPos);
			setNextSelectedPositionInt(newPos);
			checkSelectionChanged();
